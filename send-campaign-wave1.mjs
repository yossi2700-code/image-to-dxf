...
// Get first 50 users who have not opted out, ordered by registration date
...
for (const user of users) {
  ...
  try {
    ...
    sent++;
    results.push({ email: user.email, lang, status: "sent", id: res.data?.id });
    ...
    failed++;
    results.push({ email: user.email, lang, status: "failed", error: err.message });
    console.error(`❌ Failed: ${user.email} — ${err.message}`);
  }
}

console.log(`\n📊 Campaign Wave 1 Complete`);
