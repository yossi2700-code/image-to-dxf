...
## 2026-05-16 19:43 GMT+3 — AI Trace DB-backed job creation fix

תוקן מצב שבו עיבוד תמונה ב־AI Trace יכול להסתיים ברקע אך הלקוח לא מקבל תוצאה, בעיקר כאשר בקשות ה־polling נוחתות על מופע שרת אחר לפני שרשומת המשימה נשמרה למסד הנתונים. יצירת המשימה ממתינה כעת לשמירה ב־DB לפני החזרת `jobId`, וצד הלקוח מנסה שוב מספר פעמים במקרה של `404` רגעי במקום להפסיק מיד את התהליך.

## 2026-05-16 18:00 GMT+3 — Conversion storage timeout fallback

תוקן מצב שבו ההמרה עצמה מסתיימת, אבל העלאת קובץ ה־DXF/SVG לאחסון נתקעת ולכן המשתמש נשאר במסך עיבוד ללא תוצאה. קריאות האחסון מוגבלות כעת ל־8 שניות; אם האחסון איטי או לא זמין, הקריאה נכשלת עם `ETIMEDOUT` ומסלול ה־fallback הקיים מחזיר קובץ זמני מסוג `data:` להורדה מיידית במקום להשאיר את הבקשה פתוחה.
