...
export function NeedleEngravingTab({ onOpenAuth, onInsufficientTokens }: NeedleEngravingTabProps) {
  ...
  const t = {
    ...
    downloadJpeg: isRtl ? "הורד JPEG" : "Download JPEG",
    downloadTiff: isRtl ? "הורד TIFF" : "Download TIFF",
    downloadFormat: isRtl ? "הורד קובץ" : "Download File",
    tipTitle: isRtl ? "טיפ לחריטה" : "Engraving Tip",
    tipText: isRtl
      ? "לתוצאות הטובות ביותר: השתמש ב-BMP 8-bit עם תוכנת החריטה שלך. JPEG מתאים לתצוגה. TIFF מתאים לעריכה מקצועית."
      : "For best results: use BMP 8-bit with your engraving software. JPEG is for preview. TIFF is for professional editing.",
  ...
  const baseName = imageFile ? imageFile.name.replace(/\.[^.]+$/, "") : (prompt.slice(0, 30) || "engraving");

  const handleDownloadFormat = async (format: "bmp" | "jpeg" | "tiff") => {
    if (!result) return;
    setShowDownloadMenu(false);
    const urlMap = { bmp: result.bmpUrl, jpeg: result.jpegUrl, tiff: result.tiffUrl };
    const extMap = { bmp: "bmp", jpeg: "jpg", tiff: "tif" };
    const mimeMap: Record<string, "application/octet-stream" | "image/jpeg" | "image/tiff"> = { bmp: "application/octet-stream", jpeg: "image/jpeg", tiff: "image/tiff" };
    const url = urlMap[format];
    if (!url) { toast.error(isRtl ? "הפורמט אינו זמין" : "Format not available"); return; }
    const filename = `${baseName}_engraving.${extMap[format]}`;
    try {
      const response = await fetch(url);
      const blob = await response.blob();
      await saveFileAs({ blob, filename, mimeType: mimeMap[format] });
    } catch {
    ...
    <div className="flex flex-col gap-5" dir={isRtl ? "rtl" : "ltr"}>
      ...
      {status !== "loading" && (
        ...
        >
          ...
          <button
            ...
            className="w-full flex items-center justify-between text-sm font-semibold px-4 py-2.5 rounded-xl mb-2 transition-all border"
            style={showSettings
      ...
      {status === "success" && result && (
        ...
        >
          ...
          {/* Result image display — large, full width */}
          {mode === "ai" ? (
            <div className="mb-4 relative">
              <div className="rounded-xl overflow-hidden bg-black border border-gray-700" style={{ minHeight: 280 }}>
                <img src={result.previewUrl} alt="engraving preview" className="w-full object-contain" />
              </div>
              {/* Tip button overlay */}
              <button
                onClick={() => setShowTip(v => !v)}
                className="absolute top-2 right-2 w-8 h-8 rounded-full flex items-center justify-center text-yellow-500 bg-black/60 hover:bg-black/80 transition-colors"
                title={t.tipTitle}
              >
            ...
            <div className="mb-4">
              {/* Processed result — large */}
              <div className="relative mb-3">
                <p className="text-xs text-gray-400 mb-1 text-center">{isRtl ? "לאחר עיבוד" : "After processing"}</p>
                <div className="rounded-xl overflow-hidden bg-black border border-gray-700" style={{ minHeight: 260 }}>
                  <img src={result.previewUrl} alt="engraving preview" className="w-full object-contain" />
                </div>
                {/* Tip button overlay */}
                <button
                  onClick={() => setShowTip(v => !v)}
                  className="absolute top-7 right-2 w-8 h-8 rounded-full flex items-center justify-center text-yellow-500 bg-black/60 hover:bg-black/80 transition-colors"
                  title={t.tipTitle}
                >
                  <Lightbulb className="w-4 h-4" />
                </button>
              </div>
              {/* Original — smaller thumbnail */}
              <div className="flex items-center gap-2">
                <p className="text-xs text-gray-400 shrink-0">{isRtl ? "מקור:" : "Original:"}</p>
                <div className="rounded-lg overflow-hidden bg-gray-100 border border-gray-200" style={{ width: 72, height: 72, flexShrink: 0 }}>
                  <img src={imagePreview || result.previewUrl} alt="original" className="w-full h-full object-cover" />
                </div>
              </div>
            </div>
          )}
          ...
          {/* Tip tooltip */}
          {showTip && (
            <div className="flex items-start gap-2 px-3 py-2 rounded-lg mb-3 text-xs" style={{ background: '#fefce8', border: '1px solid #fde68a', color: '#92400e' }}>
          ...
          <div className="relative mb-2">
            ...
            {showDownloadMenu && (
              <div className="absolute bottom-12 left-0 right-0 rounded-xl overflow-hidden shadow-xl z-20" style={{ background: '#fff', border: '1px solid #e2e8f0' }}>
                <button onClick={() => handleDownloadFormat("bmp")} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-emerald-50 text-sm font-semibold text-gray-800 transition-colors">
                  <Download className="w-4 h-4 text-emerald-600" />
                  <div className="text-left">
                    <div>BMP 8-bit</div>
                    <div className="text-xs text-gray-400 font-normal">{isRtl ? "מומלץ לתוכנת חריטה" : "Recommended for engraving software"}</div>
                  </div>
                </button>
                <div style={{ height: 1, background: '#f1f5f9' }} />
                <button onClick={() => handleDownloadFormat("jpeg")} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-blue-50 text-sm font-medium text-gray-700 transition-colors">
                  <Download className="w-4 h-4 text-blue-500" />
                  <div className="text-left">
                    <div>JPEG</div>
                    <div className="text-xs text-gray-400">{isRtl ? "תצוגה ושיתוף" : "Preview & sharing"}</div>
                  </div>
                </button>
                <div style={{ height: 1, background: '#f1f5f9' }} />
                <button onClick={() => handleDownloadFormat("tiff")} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-purple-50 text-sm font-medium text-gray-700 transition-colors">
                  <Download className="w-4 h-4 text-purple-500" />
                  <div className="text-left">
                    <div>TIFF</div>
                    <div className="text-xs text-gray-400">{isRtl ? "עריכה מקצועית" : "Professional editing"}</div>
                  </div>
                </button>
              </div>
            )}
          ...
          <button
            onClick={handleReset}
            className="w-full mt-1 text-xs text-gray-400 hover:text-gray-600 py-1.5 transition-colors"
          >
