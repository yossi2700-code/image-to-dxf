...
export default function Admin() {
  ...
  const { data: daily, isLoading: dailyLoading } = trpc.admin.dailyActivity.useQuery(undefined, {
    enabled: !!user,
    retry: false,
  });

  const { data: recent, isLoading: recentLoading } = trpc.admin.recentEvents.useQuery(undefined, {
    enabled: !!user,
    retry: false,
  });
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50">
        <Card className="w-full max-w-sm mx-4">
          <CardContent className="p-8 flex flex-col items-center gap-4 text-center">
            ...
            <h2 className="text-xl font-bold">כניסה נדרשת</h2>
            <p className="text-sm text-muted-foreground">
              דשבורד הניהול מיועד לבעלים בלבד. יש להתחבר כדי להמשיך.
            </p>
  ...
  // Format chart data — fill missing days with zeros
  const chartData = (() => {
    if (!daily) return [];
