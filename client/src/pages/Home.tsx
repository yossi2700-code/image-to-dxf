      ...
      <div className="px-5 py-4 flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div
            ...
            style={{ background: 'rgba(0,0,0,0.1)', border: '1px solid rgba(165,180,252,0.22)' }}
          >
...
function CtaSection() {
  const { isRtl } = useLanguage();
  const { data: contactInfo } = trpc.contact.info.useQuery(undefined, { staleTime: 10 * 60 * 1000 });
  ...
  return (
    <>
        ...
        <div className="mx-auto" style={{ maxWidth: '560px' }}>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            {contactInfo?.whatsappNumber && (
              <a
                href={`https://wa.me/${contactInfo.whatsappNumber.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(isRtl ? 'שלום, אני מעוניין ב-DXF AI' : 'Hello, I am interested in DXF AI')}`}
                target="_blank"
                ...
                style={{ background: '#25D366', color: 'white', boxShadow: '0 4px 14px rgba(37,211,102,0.35)', fontSize: '0.9rem' }}
              >
            ...
            {contactInfo?.supportEmail && (
              <a
                ...
                className="inline-flex items-center gap-2 font-bold px-5 py-2.5 rounded-xl transition-all hover:opacity-90 hover:scale-105"
                style={{ background: 'rgba(255,255,255,0.15)', color: 'white', border: '1px solid rgba(255,255,255,0.3)', fontSize: '0.9rem' }}
              >
            ...
            {contactInfo?.contactPhone && (
              ...
              >
                ...
                {isRtl ? 'חייג טלפון' : 'Call us'}
              </a>
            )}
            <button
              onClick={() => { setShowMsgModal(true); setMsgSent(false); }}
              className="inline-flex items-center gap-2 font-bold px-5 py-2.5 rounded-xl transition-all hover:opacity-90 hover:scale-105"
              style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: 'white', boxShadow: '0 4px 16px rgba(99,102,241,0.4)', fontSize: '0.9rem' }}
            >
      ...
      {showMsgModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.6)' }} onClick={() => setShowMsgModal(false)}>
          <div className="w-full max-w-md rounded-2xl p-6" style={{ background: 'white' }} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold" style={{ color: '#1e1b4b' }}>{isRtl ? 'שלח הודעה' : 'Send us a message'}</h3>
              <button onClick={() => setShowMsgModal(false)} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button>
            </div>
            {msgSent ? (
              <div className="text-center py-8">
                <div className="text-4xl mb-3">✅</div>
                <p className="font-bold text-green-700">{isRtl ? 'הודעה נשלחה בהצלחה!' : 'Message sent successfully!'}</p>
                <p className="text-sm text-gray-500 mt-1">{isRtl ? 'נחזור אליך בהקדם האפשר' : 'We will get back to you soon'}</p>
              </div>
            ) : (
              <div className="flex flex-col gap-3">
                ...
                >
                  {sendMsg.isPending ? (isRtl ? 'שולח...' : 'Sending...') : (isRtl ? 'שלח הודעה' : 'Send message')}
                </button>
              </div>
            )}
          </div>
...
function ContactBar() {
  ...
  const sendMsg = trpc.contact.sendMessage.useMutation({
    ...
    onError: () => toast.error(isRtl ? 'שגיאה בשליחה' : 'Failed to send'),
  });

  return (
    <>
      <div className="container py-6" style={{ borderTop: '1px solid #e8eaf0', marginTop: '1.5rem' }}>
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-sm font-semibold" style={{ color: '#374151' }}>
            {isRtl ? 'יש שאלות? אנחנו כאן' : 'Have questions? We\'re here to help'}
          </p>
          <div className="flex flex-wrap items-center justify-center gap-2">
            {data?.whatsappNumber && (
              <a
                href={`https://wa.me/${data.whatsappNumber.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(isRtl ? 'שלום, אני מעוניין ב-DXF AI' : 'Hello, I need help with DXF AI')}`}
                ...
                style={{ background: '#25D366', color: 'white', boxShadow: '0 2px 8px rgba(37,211,102,0.3)' }}
              >
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                WhatsApp
              </a>
            ...
            {data?.contactPhone && (
              <a
                ...
                className="inline-flex items-center gap-2 font-bold px-4 py-2 rounded-xl transition-all hover:opacity-90 hover:scale-105 text-sm"
                style={{ background: '#0ea5e9', color: 'white', boxShadow: '0 2px 8px rgba(14,165,233,0.3)' }}
              >
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.81a19.79 19.79 0 01-3.07-8.67A2 2 0 012 1h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.09 8.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>
                {isRtl ? 'טלפון' : 'Call'}
              </a>
      ...
      {showMsgModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.6)' }} onClick={() => setShowMsgModal(false)}>
          <div className="w-full max-w-md rounded-2xl p-6" style={{ background: 'white' }} onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold mb-4" style={{ color: '#1e1b4b' }}>{isRtl ? 'שלח הודעה' : 'Send a message'}</h3>
            {msgSent ? (
              <div className="text-center py-6">
                <div className="text-4xl mb-3">✅</div>
                <p className="font-bold text-green-700">{isRtl ? 'ההודעה נשלחה!' : 'Message sent!'}</p>
                <button onClick={() => setShowMsgModal(false)} className="mt-4 px-4 py-2 rounded-lg text-sm font-bold" style={{ background: '#6366f1', color: 'white' }}>{isRtl ? 'סגור' : 'Close'}</button>
              </div>
            ) : (
              <div className="flex flex-col gap-3">
                <input value={msgName} onChange={e => setMsgName(e.target.value)} placeholder={isRtl ? 'שם' : 'Name'} className="border rounded-lg px-3 py-2 text-sm" style={{ borderColor: '#e5e7eb' }} />
                <input value={msgEmail} onChange={e => setMsgEmail(e.target.value)} placeholder={isRtl ? 'מייל' : 'Email'} className="border rounded-lg px-3 py-2 text-sm" style={{ borderColor: '#e5e7eb' }} />
                <textarea value={msgText} onChange={e => setMsgText(e.target.value)} placeholder={isRtl ? 'הודעה...' : 'Message...'} rows={4} className="border rounded-lg px-3 py-2 text-sm resize-none" style={{ borderColor: '#e5e7eb' }} />
                <button
                  onClick={() => sendMsg.mutate({ name: msgName, email: msgEmail, message: msgText })}
                  disabled={!msgName || !msgEmail || !msgText || sendMsg.isPending}
                  className="py-2.5 rounded-xl font-bold text-sm transition-all hover:opacity-90 disabled:opacity-50"
                  style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: 'white' }}
                >{sendMsg.isPending ? (isRtl ? 'שולח...' : 'Sending...') : (isRtl ? 'שלח' : 'Send')}</button>
              </div>
            )}
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      <main className="py-5" style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px 24px' }}>
        ...
        {/* ── Contact bar inside logged-in area ── */}
        <ContactBar />
      </main>
