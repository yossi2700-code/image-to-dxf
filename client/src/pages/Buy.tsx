...
function PurchaseTermsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="rounded-2xl shadow-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto" style={{ background: "#0f0f1a", border: "1px solid rgba(255,255,255,0.1)" }} onClick={(e) => e.stopPropagation()}>
        <div className="sticky top-0 flex justify-between items-center p-6" style={{ background: "#0f0f1a", borderBottom: "1px solid rgba(255,255,255,0.08)" }} dir={isHe ? "rtl" : "ltr"}>
          <h2 className="text-xl font-bold text-white">{isHe ? "תנאי רכישה" : "Purchase Terms & Conditions"}</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-white transition-colors w-8 h-8 flex items-center justify-center rounded-full" style={{ background: "rgba(255,255,255,0.08)" }}>
        ...
        <div className="sticky bottom-0 p-4 flex justify-end" style={{ background: "#0f0f1a", borderTop: "1px solid rgba(255,255,255,0.08)" }}>
...
export default function Buy() {
  ...
  // Card gradients and border colors per package index
  const cardGradients = [
  ...
  const cardBorders = ["#3b82f6", "#7c3aed", "#10b981", "#6366f1", "#f43f5e"];
  ...
  return (
    <div className="min-h-screen text-white" style={{ background: "#0f0f1a" }} dir={isRtl ? "rtl" : "ltr"}>
      ...
      <div className="relative max-w-5xl mx-auto px-4 py-12">
        ...
        <button
          onClick={() => navigate("/")}
          className="mb-10 inline-flex items-center gap-2 text-sm transition-colors group"
        ...
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-medium mb-6 tracking-wide uppercase" style={{ background: "rgba(99,102,241,0.15)", border: "1px solid rgba(99,102,241,0.3)", color: "#a5b4fc" }}>
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
            {isRtl ? "אסימוני עיצוב" : "Design Tokens"}
          </div>
          <h1 className="text-4xl sm:text-5xl font-black mb-4 tracking-tight" style={{ background: "linear-gradient(135deg, #a78bfa, #60a5fa, #34d399)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
            {isRtl ? "בחר את החבילה שלך" : "Choose Your Plan"}
          </h1>
          <p className="text-lg max-w-md mx-auto leading-relaxed" style={{ color: "rgba(255,255,255,0.4)" }}>
            {t("buyPageSubtitle")}
          </p>

          {isLoggedIn && balance !== null && (
            <div className="mt-6 inline-flex items-center gap-3 rounded-2xl px-6 py-3" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)" }}>
              <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: "rgba(251,191,36,0.15)", border: "1px solid rgba(251,191,36,0.3)" }}>
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" style={{ color: "#fbbf24" }}><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 14.5v-9l6 4.5-6 4.5z"/></svg>
              </div>
              <div className={isRtl ? "text-right" : "text-left"}>
                <div className="text-xs" style={{ color: "rgba(255,255,255,0.4)" }}>{t("buyCurrentBalance")}</div>
                <div className="font-bold text-white">{balance} <span className="font-normal text-sm" style={{ color: "rgba(255,255,255,0.4)" }}>{t("buyTokensLabel")}</span></div>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex justify-center mb-10">
          <div className="flex items-center gap-3 rounded-xl px-5 py-2.5 transition-colors" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)" }}>
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} style={{ color: "rgba(255,255,255,0.4)" }}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            <span className="text-sm" style={{ color: "rgba(255,255,255,0.4)" }}>{t("buySelectCurrency")}:</span>
            <select
              value={currency}
              onChange={(e) => setCurrency(e.target.value)}
              className="text-sm font-semibold focus:outline-none cursor-pointer"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-12 max-w-3xl mx-auto">
          {packages.map((p, idx) => {
            ...
            const gradIdx = idx % cardGradients.length;
            ...
            return (
              <div
                ...
                style={{
                  background: isSelected ? cardGradients[gradIdx] : "rgba(255,255,255,0.03)",
                  border: `1.5px solid ${isSelected ? cardBorders[gradIdx] : "rgba(255,255,255,0.07)"}`,
                  boxShadow: isSelected ? `0 0 30px ${cardBorders[gradIdx]}40, 0 4px 20px rgba(0,0,0,0.4)` : "0 2px 8px rgba(0,0,0,0.3)",
                  transform: isSelected ? "scale(1.02)" : "scale(1)",
                }}
              >
                {/* Badge */}
                {badge && badgeConfig[badge] && (
                  <div className="absolute top-0 left-0 right-0 py-1.5 text-center text-xs font-bold tracking-wide text-white" style={{ background: "rgba(0,0,0,0.3)" }}>
                ...
                <div className={`p-7 ${badge || (p.popular && !pDiscount) ? "pt-10" : ""}`}>
                  ...
                  <div className="mb-5">
                    <div className="flex items-end gap-2 mb-1">
                      ...
                      <span className="text-sm mb-1.5 uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.5)" }}>{t("buyTokensCount")}</span>
                    </div>
                    {(p as { label?: string | null }).label && (
                      <p className="text-sm" style={{ color: "rgba(255,255,255,0.5)" }}>{(p as { label?: string | null }).label}</p>
                    )}
                  </div>

                  {/* Divider */}
                  <div className="mb-5" style={{ borderTop: "1px solid rgba(255,255,255,0.1)" }} />

                  {/* Price */}
                  <div className="mb-5">
                    {pDiscount > 0 ? (
                      <div className="flex items-baseline gap-2">
                        <span className="line-through text-lg" style={{ color: "rgba(255,255,255,0.3)" }}>{symbol}{pPrice}</span>
                        <span className="text-3xl font-black text-white">{symbol}{pFinalPrice}</span>
                      </div>
                    ) : (
                      <span className="text-3xl font-black text-white">{symbol}{pFinalPrice}</span>
                    )}
                    <div className="text-xs mt-1" style={{ color: "rgba(255,255,255,0.4)" }}>{symbol}{pPerToken} {t("buyPerToken")}</div>
                  </div>
                  ...
                  <div className="flex items-center justify-between rounded-xl px-4 py-2.5 transition-all" style={{
                    ...
                    <span className="text-sm font-semibold" style={{ color: isSelected ? "white" : "rgba(255,255,255,0.4)" }}>
                      {isSelected ? (isRtl ? "נבחר ✓" : "Selected ✓") : (isRtl ? "בחר חבילה" : "Select plan")}
                    </span>
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} style={{ color: isSelected ? "white" : "rgba(255,255,255,0.3)" }}>
                      {isSelected
        ...
        <div className="max-w-3xl mx-auto mb-12">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              <div key={i} className="flex items-center gap-2.5 rounded-xl px-4 py-3" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
                <span style={{ color: "#34d399" }} className="flex-shrink-0">{f.icon}</span>
                <span className="text-xs font-medium" style={{ color: "rgba(255,255,255,0.6)" }}>{f.text}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-md mx-auto">
          <div className="rounded-2xl p-6" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", boxShadow: "0 8px 40px rgba(0,0,0,0.4)" }}>

            {/* Order summary */}
            <div className="mb-6">
              <h3 className="text-xs uppercase tracking-widest mb-4" style={{ color: "rgba(255,255,255,0.3)" }}>{isRtl ? "סיכום הזמנה" : "Order Summary"}</h3>
              <div className="flex items-center justify-between py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
                <div>
                  ...
                  <div className="text-xs mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>{symbol}{perToken} {t("buyPerToken")}</div>
                </div>
                <div className="text-right">
                  {discount > 0 && <div className="line-through text-sm" style={{ color: "rgba(255,255,255,0.3)" }}>{symbol}{price}</div>}
                  <div className="text-white font-bold text-xl">{symbol}{finalPrice}</div>
                  <div className="text-xs" style={{ color: "rgba(255,255,255,0.3)" }}>{currency}</div>
                </div>
              </div>
            </div>
            ...
            {/* Alerts */}
            {error && (
              <div className="mb-4 p-3 rounded-xl text-sm text-center" style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", color: "#fca5a5" }}>
                {error}
              </div>
            )}
            {isLoggedIn === false && (
              <div className="mb-4 p-3 rounded-xl text-sm text-center" style={{ background: "rgba(251,191,36,0.1)", border: "1px solid rgba(251,191,36,0.3)", color: "#fde68a" }}>
                {t("buyLoginRequired")}
              </div>
            )}
            {paypalConfigured === false && (
              <div className="mb-4 p-3 rounded-xl text-sm text-center" style={{ background: "rgba(234,179,8,0.1)", border: "1px solid rgba(234,179,8,0.3)", color: "#fef08a" }}>
            ...
            <div className="flex gap-2 mb-5 p-1 rounded-xl" style={{ background: "rgba(255,255,255,0.05)" }}>
              ...
              <div className="flex-1 py-2.5 rounded-lg flex items-center justify-center gap-2 select-none cursor-not-allowed" style={{ color: "rgba(255,255,255,0.2)" }}>
              ...
              {/* PayPal — active */}
              <div className="flex-1 py-2.5 rounded-lg flex items-center justify-center gap-2 select-none" style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", color: "white" }}>
            ...
            <button
              ...
              style={
                loading || !termsAccepted || isLoggedIn === false || paypalConfigured === false
                  ? { background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.2)", cursor: "not-allowed", border: "1px solid rgba(255,255,255,0.08)" }
                  : { background: "linear-gradient(135deg, #4f46e5, #7c3aed, #2563eb)", color: "white", boxShadow: "0 0 30px rgba(99,102,241,0.5), 0 4px 15px rgba(99,102,241,0.3)", cursor: "pointer" }
              }
              onMouseEnter={e => {
                if (!loading && termsAccepted && isLoggedIn !== false && paypalConfigured !== false) {
                  (e.currentTarget as HTMLButtonElement).style.boxShadow = "0 0 40px rgba(99,102,241,0.7), 0 6px 20px rgba(99,102,241,0.4)";
                  (e.currentTarget as HTMLButtonElement).style.transform = "scale(1.01)";
                }
              }}
              onMouseLeave={e => {
                (e.currentTarget as HTMLButtonElement).style.boxShadow = "0 0 30px rgba(99,102,241,0.5), 0 4px 15px rgba(99,102,241,0.3)";
            ...
            <div className="mt-4 flex items-center justify-center gap-2 text-xs" style={{ color: "rgba(255,255,255,0.25)" }}>
        ...
        <div className="max-w-2xl mx-auto mt-20 mb-10">
          ...
          <div className="space-y-3">
            {[
              ...
              <details key={i} className="group rounded-xl overflow-hidden" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
                <summary className="flex items-center justify-between px-5 py-4 cursor-pointer list-none">
                  <span className="font-semibold text-white text-sm">{item.q}</span>
                  <svg className="w-4 h-4 group-open:rotate-180 transition-transform flex-shrink-0 ml-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} style={{ color: "rgba(255,255,255,0.4)" }}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7"/></svg>
                </summary>
                <div className="px-5 pb-4 text-sm leading-relaxed pt-3" style={{ color: "rgba(255,255,255,0.5)", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                  {item.a}
                </div>
              </details>
            ))}
        ...
        <div className="max-w-md mx-auto pb-20">
          <div className="rounded-2xl p-8 text-center" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
            <div className="w-12 h-12 mx-auto mb-4 rounded-2xl flex items-center justify-center" style={{ background: "rgba(99,102,241,0.15)", border: "1px solid rgba(99,102,241,0.3)" }}>
              <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} style={{ color: "#a5b4fc" }}><path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z"/></svg>
            </div>
