...
function buildReminderEmail({ language = "he", name = null, unsubscribeToken = "DEMO" }) {
  ...
  const subject = isHe ? "יצרת משהו מדהים לאחרונה?" : "Created anything amazing lately?";
  const greeting = displayName
    ? (isHe ? `שלום ${displayName},` : `Hi ${displayName},`)
    : (isHe ? "שלום," : "Hi there,");
  const headline = isHe ? "יצרת משהו מדהים לאחרונה?" : "Created anything amazing lately?";
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#0f172a;padding:32px 16px;">
  <tr><td align="center">
    <table width="580" cellpadding="0" cellspacing="0" style="max-width:580px;width:100%;background:#1e293b;border-radius:20px;overflow:hidden;border:1px solid #334155;">
      ...
      <tr><td style="padding:40px 40px 32px;font-family:Arial,sans-serif;direction:${dir};">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:32px;">${featureRows}</table>
        <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:32px;">
          <tr><td style="background:linear-gradient(135deg,#022c22,#064e3b);border-radius:14px;padding:24px;border:1px solid #059669;text-align:center;">
            ...
            <a href="${SITE_URL}/free" style="display:inline-block;background:linear-gradient(135deg,#059669,#0d9488);color:white;padding:13px 32px;border-radius:50px;text-decoration:none;font-weight:900;font-size:14px;box-shadow:0 4px 16px rgba(5,150,105,0.5);">${freeCta}</a>
          </td></tr>
        </table>
        <div style="text-align:center;margin-bottom:32px;">
          <a href="${SITE_URL}" style="display:inline-block;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:white;padding:16px 44px;border-radius:50px;text-decoration:none;font-weight:900;font-size:16px;box-shadow:0 6px 24px rgba(99,102,241,0.45);">${mainCta}</a>
        </div>
      ...
      <tr><td style="background:#0f172a;padding:20px 40px;border-top:1px solid #1e293b;text-align:center;font-family:Arial,sans-serif;">
        ...
        <p style="margin:0;color:#334155;font-size:11px;"><a href="${SITE_URL}/unsubscribe?token=${unsubscribeToken}" style="color:#334155;text-decoration:underline;">${unsubText}</a></p>
      </td></tr>
    </table>
  </td></tr>
</table>
...
// Get all remaining users from offset 150
const [users] = await db.execute(`
  SELECT id, email, name, language, emailOptOut
  FROM app_users
...
const heCount = users.filter(u => u.language === "he").length;
const enCount = users.filter(u => u.language !== "he").length;
console.log(`📋 Wave 3: ${users.length} users — ${heCount} Hebrew, ${enCount} other languages`);

let sent = 0;
let failed = 0;

for (const user of users) {
  const lang = user.language === "he" ? "he" : "en";
  ...
  try {
    await resend.emails.send({
      ...
      html,
      text: lang === "he"
        ? `שלום! כנס ויצרו ב-DXF AI: ${SITE_URL} — קבצים חינם: ${SITE_URL}/free`
        : `Hi! Start creating at DXF AI: ${SITE_URL} — Free files: ${SITE_URL}/free`,
      replyTo: "support@dxfai.ai",
    });

    sent++;
    console.log(`✅ [${sent}/${users.length}] ${user.email} (${lang})`);
