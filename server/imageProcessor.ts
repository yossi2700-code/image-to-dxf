...
export function segmentsToSvg(
  ...
  for (const seg of segments) {
    lines.push(
      `<line x1="${seg.x1}" y1="${seg.y1}" x2="${seg.x2}" y2="${seg.y2}" stroke="#1a1a2e" stroke-linecap="round"/>`
    );
  }
  lines.push("</svg>");
  return lines.join("\n");
...
export function polylinesToSvg(
  ...
  for (const poly of polylines) {
    ...
    lines.push(
      `<polyline points="${pts}" fill="none" stroke="#1a1a2e" stroke-linecap="round" stroke-linejoin="round"/>`
    );
  }
  lines.push("</svg>");
  return lines.join("\n");
