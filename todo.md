...
- [x] תיקון: תמונה מקורית (source) לא נטענת בדף דיווחי בעיות — base64 עולה ל-S3 לפני שמירה
- [ ] תיקון: שוליים/layout בעיה בדף דיווחי בעיות במובייל

## תיקונים - אפריל 2026

- [x] תיקון: תמונת פורטרט חורגת מהשוליים במובייל — כרטיס SVG לא מוגבל ברוחב
- [x] תיקון: טקסט "3 גרסאות" בפורטרט — צריך להיות "1 גרסה"
- [x] תיקון: תמונה מקורית (source) לא נטענת בדף דיווחי בעיות — base64 עולה ל-S3 לפני שמירה

## מייל ברוכים הבאים - עיצוב מחדש - אפריל 2026

- [ ] עיצוב מחדש: מייל פשוט ללא תמונות/באנרים, עברית/אנגלית לפי שפת משתמש
- [ ] תוכן: כותרת "ברוך הבא [שם]", פרטי כניסה, לינק לאתר, הסבר 10 אסימונים חינם
...
## FreeDXF login redirect - אפריל 2026

- [x] כפתור התחברות/הרשמה בדף FreeDXF פותח AuthDialog בדף ונשאר בדף /free אחרי התחברות

## UI שיפורים - אפריל 2026

- [x] הסרת שורת נושאים sticky מדף הנחיתה
- [x] הסרת כפתור "קבצים בחינם" מה-header הראשי
- [x] הדגשת הבאנר "קבצי DXF בחינם לקהילה" בדף הנחיתה

## FreeDXF - עיצוב מחדש פשוט ומהיר

- [ ] תיקון תצוגת preview בכרטיסי FreeDXF
- [ ] פישוט תהליך ההורדה - לחיצה אחת, ללא סיבוך
- [ ] עיצוב מחדש של הדף - פשוט, נקי, מהיר

## עדכונים - אפריל 2026

- [x] כפתור FreeDXF בנאבר - צבע טורקיז (teal/cyan) ממש לפני לוגו dxfai
- [x] ממשק ניהול קבצים משותפים - כפתורי "אשר" ו"דחה" ישירים ללא טופס עריכה
- [x] קרדיט יוצר בדף פרטי קובץ FreeDXF (עם אווטאר ושם)
- [x] כפתור שיתוף לקהילה בדף ההיסטוריה (כפתור טורקיז עם אייקון share)
- [x] דיאלוג שיתוף לקהילה עם שדה שם יוצר אופציונלי
...
## עדכונים - אפריל 2026 (סשן 4)

- [ ] FreeDXF גלריה + דף פרטים: שינוי מילוי SVG לשחור (ללא טורקיז)
- [ ] ניהול קבצים משותפים: הוספת עריכת קרדיט + קטגוריה + שם קובץ לפני אישור
- [ ] קטגוריות בעברית בניהול עם תרגום אנגלי בדף הציבורי
- [ ] FreeDXF נאבר: כפתור תרגום 7 שפות (כמו באתר הראשי)
- [x] תיקון סיווג תמונות — ציורים/line art מסווגים כ-drawing ולא כ-object
- [x] חיזוק פרומפט drawing mode — הוראות מחמירות לנאמנות לתמונה המקורית (אסור לשנות כיוון/פרופורציות/להוסיף אלמנטים)
- [x] זיהוי אוטומטי שחור-לבן — אם התמונה כבר B&W line art, לדלג על AI ולעבור ישירות ל-Potrace
- [x] החלפת AI Create מ-OpenAI ישיר למפתח Manus Forge API
- [x] אזור אישי — תצוגת פרומפט מלא ב-AI Create (הטקסט נחתך, צריך להציג את כל הטקסט)

## שרטוט אדריכלי AI - אפריל 2026

- [x] יצירת קומפוננט ArchitecturalAiTab — שכפול AI Create עם צבע teal/slate
- [x] טבלת פרמטרים חובה: סוג שרטוט, קנה מידה, יחידות מידה, מידות (רוחב×אורך×גובה)
- [x] פרמטרים אופציונליים: עובי קירות, סגנון שרטוט (toggle)
- [x] שדה קנה מידה מותאם אישית (custom scale)
- [x] גובה חובה לסוגי שרטוט חזית/חתך
- [x] בניית prompt אדריכלי אוטומטי מהפרמטרים + תיאור חופשי
- [x] שילוב בדף הראשי — טאב "שרטוט AI" עם צבע teal
- [x] Banner הסבר בראש הטאב
- [x] 13 unit tests עוברים (validation + prompt builder)

## שיפורי שרטוט אדריכלי AI - אפריל 2026 (שלב 2)

- [ ] שיפור prompt אדריכלי: קווי CAD נקיים, ללא צלליות, 3 וריאציות סגנוניות שונות
- [ ] הוספת תמונות דמו אדריכליות בראש הטאב (תוכנית קומה, חזית, חתך)
- [ ] שמירת היסטוריה עם תיוג "arch" לשרטוטים אדריכליים
- [ ] הצגת תיוג "אדריכלי" בהיסטוריה

## שיפורי שרטוט אדריכלי - אפריל 2026 (סבב 2)
- [x] שיפור prompt לסגנון AutoCAD מקצועי עם תוויות חדרים באנגלית ומידות
- [x] הוספת שדה מספר חדרי שינה (מופיע רק לתוכנית קומה)
- [x] הצגת פיצ'ר שרטוט אדריכלי בדף הבית עם כרטיס AutoCAD
- [x] תיוג arch_ai בהיסטוריה עם אייקון Building2 בצבע cyan
...
## תיקון קווים כפולים AI יצירה - אפריל 2026
- [x] blur(2.0) הוסף והוחלף ב-blur(1.5) + הגדלה ל-2048 לשמירת פרטים
## תיקון אותיות קטנות בוקטור - אפריל 2026
- [x] הגדלה ל-2048 + blur(1.5) במקום blur(2.0) לשמירת פרטים קטנים כמו אותיות

## הסרת פיצ'רים לא מוכנים - אפריל 2026
...
- [x] עיצוב מחדש דיאלוג "יותר מדי פנים" — כותרת נכונה + עיצוב ידידותי מודרני
- [x] תיקון טקסטי סטטוס פורטרט שמופיעים בעברית גם כשהשפה אנגלית
- [x] תיקון דיאלוג הורדה PNG: קיצור הבאנר, X גלוי, כפתור שיתוף גלוי ללא גלילה
- [x] קיצור נוסף דיאלוג הורדה — הסרת preview, slider קומפקטי, כפתורים קטנים יותר
- [x] בדיקת ותיקון בעיית קווים עבים ב-vectorization (קרה היום)
- [x] תיקון adaptive threshold — תמונות עם רקע כהה יוצאות שחורות לגמרי
- [x] הגנה מפני לחיצה כפולה בכפתורי AI — מניעת חיוב כפול באסימונים
- [ ] פיצר תבליט: הוספת ייצוא TIFF 16-bit, רזולוציה 3000-4000px, slider עומק
- [ ] AI מתמונה: שיפור prompt לדיוק 100% כמו המקור (ללא יצירתיות)
- [ ] AI מטקסט: ה-AI יצייר רק תמונה ויזואלית ללא טקסט בפלט

## שיפור עיבוד שרטוטים לא ברורים

- [ ] עיבוד מקדים sharp: ניגודיות + חידוד לפני שה-AI רואה את התמונה
- [ ] הנחיה ל-AI לצייר קווים חזקים גם כשהמקור חלש/מטושטש

## תיקון Portrait - גוף מינימלי

- [x] Portrait: הוסף צוואר + כתפיים + חלק עליון חזה לפלט — ראש מנותק נראה מגוחך

## תיקון מילוי שחור בפורטרט

- [x] Portrait: בגדים כהים מתמלאים שחור — צריך קווי מתאר בלבד, ללא fill

## תיקון modal "יותר מ-2 פנים"
- [x] Modal: כותרת שגוייה "לא זוהו פנים" — צריך להיות "יותר מ-2 פנים בתמונה" (תוקן: השרת לא העביר errorCode)
- [x] Modal: תוכן מותאם — הסבר שפורטרט תומך עד 2 פנים + אפשרות לעבור לתמונה לקווים
- [x] Modal: עיצוב כפתורים — צבע/גבול ברור שאפשר ללחוץ עליהם
...
## שינויים - אפריל 2026 (סשן נוכחי)

- [x] הוספת recordUserAction בהורדת קובץ מאתר החינם (freedxfApi.ts)
- [x] אתר החינם: שינוי "טען עוד X קבצים" ל-"טען עוד קבצים" (ללא מספר)
- [x] אתר החינם: הסרת תצוגת כמות קבצים מה-stats row (נשאר רק קטגוריות)
- [x] אתר החינם: הסרת "מציג X מתוך Y" מה-section header

## שיפור הורדה בדף חינם
- [ ] שינוי כפתור "הורד DXF" ל"הורד קובץ" עם דיאלוג בחירת פורמט (DXF, SVG, PNG)

## דחוף - עדכוני UX (26 אפריל 2026)

- [ ] עיצוב דיאלוג אין-קרדיטים מאוחד ויפה - להראות למשתמש לפני החלפה
- [ ] החלפת הודעת אין-אסימונים בתמונה לקווים
- [ ] החלפת הודעת אין-אסימונים בפורטרט
- [ ] החלפת הודעת אין-אסימונים ב-AI יצירה
- [ ] תיקון לוגיקת העלאה בתמונה לקווים: העלאה -> בודק תמונה -> auto-start
- [ ] שינוי שם אסימונים לקרדיטים בכל האתר
- [ ] הוספת אייקון קרדיטים בכל מקום שמוצג מספר קרדיטים

## עדכוני UX דחופים (26 אפריל 2026)
- [x] דיאלוג אין-קרדיטים מאוחד - עיצוב gradient סגול-כחול, ללא חארטה, לינק לרכישה
- [x] הסרת "נסה תמונה לקווים" מפורטרט
- [x] הסרת הודעת מייל מ-AiTrace (token error מוצג רק בדיאלוג המאוחד)
- [x] הסרת הודעת מייל מ-AiGenerator (token error מוצג רק בדיאלוג המאוחד)
- [x] שינוי "אסימונים" → "קרדיטים" בכל האתר (translations.ts + כל הקומפוננטות)
...
## תיקון דף מעקב - אפריל 2026
- [x] הסר מגבלת 200 משתמשים — הצג את כל המשתמשים (עד 5000)
- [x] הוסף מעקב page_view לכל ביקור בדף (כולל משתמשים לא מזוהים)
- [x] תקן היסטוריית לחיצות — page_view נרשם אוטומטית לכל משתמש
- [ ] ודא שהמעקב עובד על הגרסה החיה (אחרי Publish)

## מיון לפי פעולה אחרונה - אפריל 2026
- [x] מיין משתמשים לפי lastClickAt DESC (פעולה אחרונה ראשונה)
- [x] הצג זמן פעולה אחרונה בולט בכרטיס כל משתמש

## מרווח קווים בפורטרט - אפריל 2026
- [x] הגדל מרווח ברירת מחדל בין קווים בפורטרט לצורך כרסום CNC (1.5mm ברירת מחדל)
- [x] הוסף סליידר מרווח קווים (0.2–3mm) גלוי תמיד בממשק הפורטרט

## תיקון איכות DXF פורטרט - אפריל 2026
...
## שיפור פורטרט - אפריל 2026 (session 3)
- [x] תקן flow - העלאת תמונה לא מתחילה עיבוד אוטומטי, צריך כפתור "עבד"
- [x] קווים עבים יותר ב-SVG ו-DXF - שינוי מ-skeleton ל-potrace outline + prompt חדש לנאמנות מקסימלית

## בדיקת בהירות תמונה - אפריל 2026
- [x] הוסף בדיקת בהירות לפני עיבוד - אם התמונה לא ברורה, הצג הודעת שגיאה ולא ממציא תוכן

## תיקון תמונה ריקה - אפריל 2026
- [x] הוסף בדיקת תמונה ריקה אחרי AI - אם ה-AI מחזיר תמונה לבנה, נסה שוב עם prompt פשוט יותר, ואם גם זה נכשל - הצג שגיאה ברורה

## תיקון פורטרט - שתי פנים וקווים דקים - אפריל 2026
- [x] תיקון פורטרט: כשיש שתי פנים בתמונה - עכשיו מעבד כל פנים בנפרד
- [x] תיקון פורטרט: קווים עבים יותר - שיפור potrace + prompt חדש

## תיקון זיהוי פנים כפול - אפריל 2026
- [x] תיקון: LLM מזהה 2 פנים בתמונה עם פנים אחת - חוזק prompt + סינון לפי גודל
...
## חריטת מחט יהלום — שילוב מלא
- [x] הוספת כרטיס לדף הבית עם תמונת לפני/אחרי
- [x] חיבור ל-10 קרדיטים לכל עיבוד
- [x] חיבור למחירון בפאנל הניהול
- [x] תמונת לפני/אחרי (נוף → סימולציית חריטה על גרניט)

## חריטת תמונה — שדרוג לטאב inline עם שני מצבים

- [x] עדכון תמונת כרטיס חריטת תמונה לרכב אספנות (מוסטנג 1969)
- [x] יצירת NeedleEngravingTab.tsx — קומפוננטה inline בסגנון AiTraceTab
- [x] מצב העלאת תמונה — drag & drop + preview + עיבוד
- [x] מצב יצירה עם AI — textarea לתיאור, AI יוצר תמונה ומעבד לBMP
...
## תיקון פיצ'ר חריטת תמונה

- [ ] תיקון שגיאת עיבוד בפיצ'ר חריטת תמונה (NeedleEngravingTab)
- [ ] הוספת מחיר/עלות אסימונים לפיצ'ר חריטת תמונה בדף המחירון
