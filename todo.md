...
## עיצוב מחדש + תיקון קווים - פברואר 2026

- [ ] עיצוב כפתורי טאב — מודגשים עם אייקונים (AI יצירה, AI מתמונה, העלאת תמונה)
- [ ] תיקון קווים בהעלאת קובץ ישיר — שימוש באותה שיטה כמו AI Generate
- [ ] עיצוב כללי לעמוד — hero section שממחיש תמונה→DXF בסגנון קלאסי מקצועי

## ייצוא PDF + הגדרות ניהול - פברואר 2026

- [ ] ייצוא PDF — כפתור "הורד PDF" בדיאלוג ההורדה (SVG → PDF בצד שרת)
- [ ] פאנל הגדרות בדשבורד ניהול — שינוי PIN, מכסה יומית, הודעת ברוכים הבאים, מצב תחזוקה

## ייצוא PDF + הגדרות ניהול

- [x] ייצוא PDF וקטורי מדיאלוג ההורדה (server-side SVG→PDF עם pdf-lib)
- [x] טאב "הגדרות" בדשבורד הניהול
- [x] שינוי קוד גישה (PIN) מהדשבורד — נשמר ב-DB, מחליף את ה-env
- [x] הודעת ברוכים הבאים — ניתנת לעריכה מהדשבורד
- [x] הגדרות אסימונים — מספר אסימונים למשתמש חדש + מכסה יומית חינמית
- [x] מצב תחזוקה — toggle שחוסם משתמשים רגילים
- [x] טבלת app_settings ב-DB (migration 0011)
- [x] 18 בדיקות חדשות לPDF ולהגדרות (108 סה"כ)

## דיאלוג שמירה בהיסטוריה

- [ ] דיאלוג שמירה בהיסטוריה — בחירת שם קובץ + סוג (DXF / PDF A4)

## תיקון באג מובייל - DxfDownloadDialog

- [ ] תיקון: דיאלוג שמירת DXF חותך תוכן במובייל — תמונה ממלאת מסך, כפתורים נעלמים

## עיצוב AI קלאסי

- [x] עיצוב AI קלאסי: gradient header כחול-סגול, כפתורים זוהרים, כרטיסים מעוצבים, רקע עדין

## תיקון באגים - דיאלוג שמירה

- [x] שם קובץ ברירת מחדל ארוך מדי — קוצר ל-3 מילים ראשונות, מקסימום 30 תווים
- [x] PDF לא נשמר — תוקן: pdfRoute עכשיו תומך ב-<path> elements (M/L/C/Q/Z) שה-AI מייצר

## תיקון באג - שם קובץ עברי ב-PDF

- [x] PDF לא מוריד כשהשם בעברית — תוקן: RFC 5987 encoding (filename*=UTF-8'' + encodeURIComponent)
